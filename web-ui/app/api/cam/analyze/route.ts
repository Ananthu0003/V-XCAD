...
export async function POST(request: Request): Promise<Response> {
	...
	if (!authUserId) {
		return NextResponse.json(
			{ error: { message: 'Authentication required.', hint: 'Log in to analyze features.' } },
			{ status: 401 }
		);
	}
	...
	if (targetSessionId && typeof targetSessionId === 'string') {
		try {
			...
			console.error('[CAM Analyze] Failed to verify session ownership:', dbError);
		}
	}

	let upstream: Response;
	try {
		upstream = await fetchWithTimeout(`${getFastApiUrl()}/cam/analyze`, {
			method: 'POST',
	...
	if (!upstream.ok || !payload) {
		return NextResponse.json(
			{ error: { message: payload?.error?.message || 'Feature analysis failed.' } },
