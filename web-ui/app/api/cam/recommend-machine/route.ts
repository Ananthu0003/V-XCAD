...
export async function POST(request: Request): Promise<Response> {
	...
	if (!authUserId) {
		return NextResponse.json(
			{ error: { message: 'Authentication required.', hint: 'Log in for machine recommendation.' } },
			{ status: 401 }
		);
	}
	...
	try {
		upstream = await fetchWithTimeout(`${getFastApiUrl()}/cam/recommend-machine`, {
			method: 'POST',
	...
	if (!upstream.ok || !payload) {
		return NextResponse.json(
			{ error: { message: payload?.error?.message || 'Machine recommendation failed.' } },
