...
export async function POST(request: Request): Promise<Response> {
	...
	if (!authUserId) {
		return NextResponse.json(
			{ error: { message: 'Authentication required.', hint: 'Log in to auto-plan operations.' } },
			{ status: 401 }
		);
	}
	...
	if (targetSessionId && typeof targetSessionId === 'string') {
		try {
			...
			console.error('[CAM Auto-Plan] Failed to verify session ownership:', dbError);
		}
	}

	let upstream: Response;
	try {
		upstream = await fetchWithTimeout(`${getFastApiUrl()}/cam/auto_plan`, {
			method: 'POST',
	...
	if (!upstream.ok || !payload) {
		return NextResponse.json(
			{ error: { message: payload?.error?.message || 'Auto-plan failed.' } },
