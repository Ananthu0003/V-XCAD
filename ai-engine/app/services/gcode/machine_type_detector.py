    ...
    def detect_machine(self, topology_info: Dict[str, Any], features: List[Dict[str, Any]]) -> str:
        ...
        * mill_turn (Mill-turn part)
        ...
        try:
            ...
            rec_res = engine.recommend_machine(features=features, topology_info=topology_info)
            part_type = rec_res.detectedPartType
            if part_type in ["turned"]:
                return "lathe"
            elif part_type in ["mill_turn"]:
                return "mill_turn"
            return "3_axis_mill"
        except Exception:
