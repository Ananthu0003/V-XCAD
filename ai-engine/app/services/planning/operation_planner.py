...
class OperationPlanner:
    ...
    def _map_feature_to_operation(self, feature: Dict[str, Any], machine_type: str) -> CamOperation:
        ...
        if not op_type:
            # Fallback to dynamic substring matching for lathe parts generated by LLM
            feat_lower = feat_type.lower() if feat_type else ""
            if machine_type in ("lathe", "mill_turn"):
                if "face" in feat_lower:
                    op_type = "facing_turning"
                elif any(kw in feat_lower for kw in ("dia", "od", "shaft", "cylinder", "turn")):
                    op_type = "od_turning"
                elif any(kw in feat_lower for kw in ("bore", "id")):
                    op_type = "id_boring"
                elif "groove" in feat_lower:
                    op_type = "grooving"
